...
const addUpdateForm = ref<ExecutionListenerForm>({
  event: 'start',
  type: 'class',
  value: '',
  fields: [],
...
const rules: FormRules<Required<ExecutionListenerForm>> = {
 ...
 * 打开修改执行监听器 Dialog
 */
const openUpdateDialog = (index: number, row: ExecutionListenerForm) => {
  visibleStatus.value = true
...
const addListenerExecution = () => {
  ...
  addUpdateFormRef.value?.validate(async (valid) => {
    if (valid) {
      ...
      if (dataIndex.value < 0) {
        useMessage().error('新增执行监听器失败')
      } else {
        useMessage().error('修改执行监听器失败')
      }
      loadingStatus.value = false
    }
  })
 ...
 * @param index 索引
 */
const removeListenerExecution = (index: any) => {
  useMessageBox()
    .confirm('是否要删除执行监听器')
    .then(() => {
...
<template>
  <el-collapse-item name="bpmn-panel-listener-execution">
    <template #title>
      <div class="flex-center">
        <bpmn-panel-title class-name="flex-1" name="执行监听器">
          <BellFilled />
        </bpmn-panel-title>
        <el-tag size="small" round effect="dark" class="mr-1">{{ tableData?.length || 0 }}</el-tag>
      </div>
    </template>
    <el-table :data="tableData" size="small" border height="200" empty-text="暂无执行监听器,请添加执行监听器！">
      ...
      <!-- @vue-generic {ExecutionListenerForm} -->
      <el-table-column label="操作" fixed="right" width="120">
        <template #default="{ $index, row }">
          <el-button type="success" size="small" text @click="openUpdateDialog($index, row)">修改</el-button>
          <el-button type="danger" size="small" text @click="removeListenerExecution($index)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="w-full flex justify-around">
      <el-button class="w-full mt-15px" type="primary" :icon="Plus" @click="openAddDialog()">内置执行监听器</el-button>
      <el-button class="w-full mt-15px" type="primary" :icon="Plus" @click="openAddDialog()">自定义执行监听器</el-button>
    </div>
    <el-drawer
      v-model="visibleStatus"
      :title="dataIndex < 0 ? '新增执行监听器' : '修改执行监听器'"
      size="45%"
      destroy-on-close
      append-to-body
    ...
    >
      ...
      >
        <el-form-item label="事件类型" prop="event">
          <el-select v-model="addUpdateForm.event" placeholder="请选择事件类型">
            <el-option value="start" label="开始" />
            <el-option value="task" label="启用" />
            <el-option value="end" label="结束" />
          </el-select>
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button :loading="loadingStatus" size="default" type="primary" @click="addListenerExecution()">提交</el-button>
      </template>
    </el-drawer>
  </el-collapse-item>
</template>
