...
import type { FlowSequenceQueryRequest, FlowSequenceResponse } from '@/service/model/workflow/sequence.model'
import { queryFlowSequencePage, removeFlowSequenceById, removeFlowSequenceByIdBatch } from '@/service/api/workflow/sequence.api'
...
const xhtSequenceFrom = defineAsyncComponent(() => import('@/views/workflow/sequence/components/sequence-form.vue'))
const flowSequenceFormRef = useTemplateRef('flowSequenceFormRef')
const queryFormRef = useTemplateRef<FormInstance>('queryFormRef')

const state = reactive<TableQueryPageState<FlowSequenceQueryRequest, FlowSequenceResponse>>({
  queryParams: {}, // 查询参数
  total: 0, // 总条目数
  pages: 0, // 总页数
...
const { handlePageQuery, handleSelectionChange } = useTableQueryPageHooks<FlowSequenceQueryRequest, FlowSequenceResponse>(
  state,
  queryFlowSequencePage
)
const { queryParams } = toRefs(state)

const columnOption = ref<ColumnConfig<FlowSequenceResponse>>({
  ...flowSequenceColumnOption,
})
 ...
 */
const handleAdd = () => {
  flowSequenceFormRef.value?.show('create', null)
}

/**
 * 处理编辑流程扩展-流程序列号
 */
const handleEdit = (row: FlowSequenceResponse) => {
  flowSequenceFormRef.value?.show('update', row.id)
}

/**
 * 处理删除流程扩展-流程序列号
 */
const handleDelete = (row: FlowSequenceResponse) => {
  state.loadingStatus = true
  useMessageBox()
    .confirm('此操作将永久删除流程扩展-流程序列号, 是否继续?')
    .then(async () => {
      await removeFlowSequenceById(row.id)
      await handlePageQuery()
      useMessage().success('删除流程扩展-流程序列号成功!')
    })
    .finally(() => {
...
const handleBatchDelete = () => {
  ...
  if (!ids || ids.length <= 0) {
    useMessage().error('请选择流程扩展-流程序列号数据')
  }
  state.loadingStatus = true
  useMessageBox()
    .confirm(`此操作将批量删除${ids.length}个流程扩展-流程序列号, 是否继续?`)
    .then(async () => {
      await removeFlowSequenceByIdBatch(ids)
      await handlePageQuery()
      useMessage().success('批量删除流程扩展-流程序列号成功!')
    })
    .finally(() => {
      state.loadingStatus = false
    })
...
<template>
  <div class="xht-view-container">
      ...
      <el-row v-else>
        ...
        <el-col :xl="4" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="是否循环" prop="isCycle">
            <el-input v-model="queryParams.isCycle" clearable :maxlength="1" show-word-limit placeholder="请输入是否循环" />
          </el-form-item>
        </el-col>
        <el-col :xl="4" :lg="6" :md="8" :sm="12" :xs="24">
    ...
    <table-tool-bar
      ...
      @refresh="resetQuery"
    >
      <el-button :icon="Plus" size="small" type="primary" @click="handleAdd" v-authorization="['xht:flow:sequence:create']">新增</el-button>
      <el-button
        :icon="Edit"
        size="small"
        type="success"
        ...
        v-authorization="['xht:flow:sequence:update']"
      >
        修改
      </el-button>
      <el-button
        ...
        v-authorization="['xht:flow:sequence:remove']"
      >
        批量删除
      </el-button>
    </table-tool-bar>
    ...
    >
      ...
      <el-table-column v-if="columnOption.sequenceName?.visible" label="序列名称" prop="sequenceName" min-width="220" />
      <el-table-column v-if="columnOption.sequenceCode?.visible" label="序列编码" prop="sequenceCode" min-width="220" />
      <el-table-column v-if="columnOption.sequenceFormat?.visible" label="序列格式" prop="sequenceFormat" min-width="260" />
      <el-table-column v-if="columnOption.minValue?.visible" label="最小值" prop="minValue" min-width="100" />
      ...
      <el-table-column label="操作" fixed="right" width="220">
        <template #default="{ row }">
          <el-space wrap class="flex-center">
            <el-button :icon="Edit" link type="success" @click="handleEdit(row)" v-authorization="['xht:flow:sequence:update']">修改</el-button>
            <el-button :icon="Delete" link type="danger" @click="handleDelete(row)" v-authorization="['xht:flow:sequence:remove']">删除</el-button>
          </el-space>
        </template>
      </el-table-column>
    </el-table>
    ...
    />
    <xht-sequence-from ref="flowSequenceFormRef" @success="handlePageQuery" />
  </div>
</template>
